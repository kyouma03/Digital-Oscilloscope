...
//Please First uncomment/comment the display driver lines, which you are using
...
#define SCREEN_WIDTH   128              // display display width
#define SCREEN_HEIGHT   64              // display display height
...
//Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, display_RESET);   // device name is display
Adafruit_SH1106 display(display_RESET);        // use this when SH1106
...
void setup() {
  ...
  //display.begin(SSD1306_SWITCHCAPVCC, 0x3C); // select 3C or 3D (set your display I2C address)
  display.begin(SH1106_SWITCHCAPVCC, 0x3C);  // use this when SH1106
...
void loop() {
  ...
  display.display();                       // send screen buffer to display (37ms)
...
void writeCommonImage() {                 //  Common screen image drawing
  display.clearDisplay();                    //  erase all(0.4ms)
  if (fftMode == true) return;            // no need for the FFT display
  display.setTextColor(WHITE);               // write in white character
  display.drawFastVLine(26, 9, 55, WHITE);   // left vartical line
  display.drawFastVLine(127, 9, 3, WHITE);   // right vrtical line up
  display.drawFastVLine(127, 61, 3, WHITE);  // right vrtical line bottom
  ...
  display.drawFastHLine(123, 9, 5, WHITE);   // right side Max value auxiliary mark
  display.drawFastHLine(123, 63, 5, WHITE);

  for (int x = 26; x <= 128; x += 5) {
    display.drawFastHLine(x, 36, 2, WHITE);  // Draw the center line (horizontal line) with a dotted line
  }
  for (int x = (127 - 25); x > 30; x -= 25) {
    for (int y = 10; y < 63; y += 5) {
      display.drawFastVLine(x, y, 2, WHITE); // Draw 3 vertical lines with dotted lines
    }
  }
}
...
void dataAnalize() {                       //  get various information from wave form
  ...
  // calculate average
  dataAve = (sum) / 20;               // Average value calculation (calculated by 10 times to improve accuracy)
...
//  display.setTextSize(2);                  // at double size character
  display.setTextColor(WHITE);
  display.println(F("Oscilloscope"));       // Title(Poor Man's Osilloscope, RadioPench 1)
  display.println(F("EDL"));          // this for SH1106
  display.display();                         // actual display here
  delay(1500);
  display.clearDisplay();
  display.setTextSize(1);                    // After this, standard font size
}

void dispHold() {                         // display "Hold"
  display.fillRect(42, 11, 24, 8, BLACK);    // black paint 4 characters
...
void dispInf() {                          //   Display of various information
...
#else
  if (scopeP == 3) {                      // if scoped
    display.drawFastHLine(0, 7, 14, WHITE); // display scoped mark at the bottom
    display.drawFastVLine(0, 5,  2, WHITE);
    display.drawFastVLine(13, 5,  2, WHITE);
  }
#endif

  //  vertical sensitivity
  display.setCursor(15, 0);                  // around top left
  display.print(vScale);                     // vertical sensitivity value
  if (scopeP == 0) {                      // if scoped
    display.drawFastHLine(13, 7, 27, WHITE); // display scoped mark at the bottom
    display.drawFastVLine(13, 5,  2, WHITE);
    display.drawFastVLine(39, 5,  2, WHITE);
  }

  //  horizontal sweep speed
  display.setCursor(42, 0);                  //
  display.print(hScale);                     // display sweep speed (time/div)
  if (scopeP == 1) {                      // if scoped
    display.drawFastHLine(40, 7, 33, WHITE); // display scoped mark
    display.drawFastVLine(40, 5,  2, WHITE);
    display.drawFastVLine(72, 5,  2, WHITE);
  }
  ...
  //  trigger polarity
  display.setCursor(75, 0);                  // at top center
  if (trigD == 0) {                       // if positive
    display.print(char(0x18));               // up mark
  } else {
    display.print(char(0x19));               // down mark              
  }
  if (scopeP == 2) {                      // if scoped
    display.drawFastHLine(72, 7, 11, WHITE); // display scoped mark
    display.drawFastVLine(72, 5,  2, WHITE);
    display.drawFastVLine(82, 5,  2, WHITE);
  }

  // average voltage
  if (inMode == 0) {                      // DC if DC mode
    display.setCursor(86, 0);
    display.print(F("av"));                  // av : average
    ...
    display.setCursor(86, 0);
    display.print(F("rm"));                  // rm : rms root mean square
  ...
  display.setCursor(98, 0);                  // at top right
  display.print(chrBuff);                    //  display voltage 
  display.print(F("V"));

  //  display frequency, duty % or trigger missed
  if (trigSync == false) {                // If trigger point can't found
    display.fillRect(92, 14, 24, 8, BLACK);  // black paint 4 character
    display.setCursor(92, 14);               //
    display.print(F("unSync"));              // display Unsync
  } else {
    display.fillRect(91, 12, 25, 9, BLACK);  // erase Freq area
    display.setCursor(92, 13);               // set display location
    if (waveFreq < 9.9995) {              // if less than 9.9995Hz
      display.print(waveFreq, 2);            // display 9.99Hz
      display.print(F("Hz"));
    } else if (waveFreq < 99.995) {       // if less than 99.995Hz
      display.print(waveFreq, 1);            // display 99.9Hz
      display.print(F("Hz"));
    } else if (waveFreq < 999.95) {       // if less than 999.95Hz
      display.print(waveFreq, 1);            // display 999.9H
      display.print(F("H"));
    } else if (waveFreq < 9995.0) {       // if less than 9.995kHz
      display.print((waveFreq / 1000.0), 2); // display 9.99kH
      display.print(F("kH"));
    } else if (waveFreq < 99950.0) {      // if less than 99.95kHz                         // if more
      display.print((waveFreq / 1000.0), 1); // display 99.9kH
      display.print(F("kH"));
    } else {                              // if more
      display.print((waveFreq / 1000.0), 0); // display 999kHz
      display.print(F("kHz"));
    }
    display.fillRect(97, 21, 25, 10, BLACK); // erase Freq area (as small as possible)
    display.setCursor(98, 23);               // set location
    display.print(waveDuty, 1);              // display duty (High level ratio) in %
    display.print(F("%"));
  }
  ...
  display.print(chrBuff);                    //  display Max value
  ...
  display.print(chrBuff);                    //  display the value
  ...
  //  display.fillRect(40, 12, 25, 9, BLACK);    //  
  //  display.setCursor(40, 13);                 //
  //  display.print(hRange);                     // 
}

void plotData() {                    //  plot waveform on display
  long y1, y2;
  if (fftMode == true) {                            // FFT
      plotFFT();
  } else if (hRange <= 9 || hRange == 12 || hRange > 14) {          //  noromal plot
    for (int x = 0; x <= 98; x++) {
      y1 = map(waveBuff[x + trigP - 50], rangeMax,  rangeMin, 63, 9); // convert to plot address
      y1 = constrain(y1, 9, 63);                                     // Crush(Saturate) the protruding part
      y2 = map(waveBuff[x + trigP - 49], rangeMax,  rangeMin, 63, 9); // to address calucurate
      y2 = constrain(y2, 9, 63);                                     //
      display.drawLine(x + 27, y1, x + 28, y2, WHITE);                  // connect between point
    }
  } else if (hRange == 10 || hRange == 13) {         // 100us2 zoom 2X when 100us range
    for (int x = 0; x <= 49; x++) {
      y1 = map(waveBuff[x + trigP - 25], rangeMax,  rangeMin, 63, 9); // convert to plot address
      y1 = constrain(y1, 9, 63);                                     // Crush(Saturate) the protruding part
      y2 = map(waveBuff[x + trigP - 24], rangeMax,  rangeMin, 63, 9); // to address calucurate
      y2 = constrain(y2, 9, 63);                                     //
      display.drawLine(x * 2 + 27, y1, x * 2 + 29, y2, WHITE);              // connect between point
    }
  } else if (hRange == 11 || hRange == 14) {        // 50us4 zoom 4x when 50us range
    for (int x = 0; x <= 24; x++) {
      y1 = map(waveBuff[x + trigP - 13], rangeMax,  rangeMin, 63, 9); // convert to plot address
      y1 = constrain(y1, 9, 63);                                     // Crush(Saturate) the protruding part
      y2 = map(waveBuff[x + trigP - 12], rangeMax,  rangeMin, 63, 9); // to address calucurate
      y2 = constrain(y2, 9, 63);                                     //
      display.drawLine(x * 4 + 27, y1, x * 4 + 31, y2, WHITE);              // connect between point
    }
  }
}
...
void battVolt() {                         //  Battery voltage measure (this for pen osillo)
  ...
  while (1) {                             // do forever
    ...
    display.clearDisplay();                  // all erase screen(0.4ms)
    display.setTextColor(WHITE);             // write in white character
    display.setCursor(20, 16);               //
    display.setTextSize(1);                  // standerd size character
    display.println(F("Battery voltage"));
    display.setCursor(35, 30);               //
    display.setTextSize(2);                  // double size character
...
void dmm5V() {                             //  5V  digital voltmeter 5V range
  ...
  while (1) {                              // 
    digitalWrite(13, HIGH);                // flash LED
    display.clearDisplay();                   // erase screen (0.4ms)
    display.setTextColor(WHITE);              // write in white character
    display.setCursor(0, 0);                  //
    display.setTextSize(1);                   // by standerd size character
    ...
    if (digitalRead(7) == HIGH) {          // DC   if switch is DC mode
      ...
      display.println(F("DC DVM 5V range"));  //
      ...
      display.println(F("AC DVM 5V range"));
      display.setTextSize(2);                 // double size character
      dtostrf(volt, 4, 2, chrBuff);        // foromat x.xx
      display.setCursor(20, 16);              //
      display.print(chrBuff);                 //  display rms voltage
      display.println(F("Vrms"));
      dtostrf(vPP, 4, 2, chrBuff);         // format x.xx
      display.setCursor(20, 38);              //
      display.print(chrBuff);                 // P-P
      display.println(F("Vpp"));
    }
    display.display();                        //  actual display here
...
void dmm50V() {                            //  5V  digital voltmeter 5V range
  ...
  while (1) {                              // forever,
    ...
    if (digitalRead(7) == HIGH) {          // DC   if awitch is DC mode
      ...
      display.println(F("DC DVM 50V range")); //
      display.setCursor(20, 22);              //
      display.setTextSize(2);                 // double size character
      ...
      display.println(F("AC DVM 50V range"));
      display.setTextSize(2);                  // double size character
...
void draw_scale() {
  ...
  if (nyquist > 999.0) {
    ...
    if (nyquist > 99.5) {
      display.setCursor(58, ylim); display.print(nyquist/2,0);display.print(F("k"));
      display.setCursor(104, ylim); display.print(nyquist,0);
    } else if (nyquist > 9.95) {
      display.setCursor(58, ylim); display.print(nyquist/2,0);display.print(F("k"));
      display.setCursor(110, ylim); display.print(nyquist,0);
    } else {
      display.setCursor(52, ylim); display.print(nyquist/2,1);display.print(F("k"));
